	...
	try {
		...
		if ('Notification' in window) {
			...
			if (Notification.permission === 'granted') {
				...
				const platformNames = unreadEntries
					.map(([platform]) => {
						const names = {
							messenger: 'Messenger',
							whatsapp: 'WhatsApp',
...
// Improved keyboard shortcuts - support all platforms
	...
	document.addEventListener('keydown', (event) => {
		if (event.metaKey || event.ctrlKey) {
			...
			const visibleTabs = tabs.filter(tab => {
				...
				// Only include welcome, settings, and enabled apps
				return platform && (platform === 'welcome' || platform === 'settings' || appState.apps[platform]?.enabled);
			});
			...
			if (event.key === 'Tab') {
				event.preventDefault();
				const nextIndex = (currentIndex + (event.shiftKey ? -1 : 1) + visibleTabs.length) % visibleTabs.length;
		...
		if (isEnabled) {
			...
			const dropHandler = (e) => {
				...
				if (draggedIndex !== -1 && targetIndex !== -1 && draggedIndex !== targetIndex) {
					logger.log(`Moving ${draggedButton.getAttribute('data-platform')} from position ${draggedIndex} to ${targetIndex}`);
...
window.addEventListener('DOMContentLoaded', () => {
	...
	// Setup keyboard shortcuts after everything is initialized
	...
	if (appState.settings.globalNotifications && 'Notification' in window && Notification.permission === 'default') {
		Notification.requestPermission();
	}
